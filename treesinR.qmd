...
\
(Note: I have seen advice to use `minsplit=0` to fit "stumps", trees with a single split. That does not seem
to work. I could not debug what was happening as the pertinent code is in C
and not accessible with `R` debugging tools.)
...
recommended in some places does not seem to work in guaranteeing a single split).
