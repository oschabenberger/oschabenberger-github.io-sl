...
Overall, the advantages outweigh the added complications of nonlinear model fitting. @Schabenberger2001 give numerous examples why nonlinear regression models are preferable over linear ones.
...
::::
::::{.example-container}
**Parsimony**. To achieve curvature with inflection in a polynomial model requires at least a model of third degree ($x^3$). Including the lower order terms, this model has 4 parameters. The nonlinear model $\Exp[Y] = 1-\exp\{-\beta x^\alpha\}$ achieves inflection with only two parameters.
...
The mineralization potential of soil can be modeled as $\Exp[Y] = \theta_0 (1-\exp\{-\theta_1 t\})$ where $\theta_0$ is the maximum amount and $0 \le 1-\exp\{-\theta_1 t\} \le 1$ is the proportional mineralization over time (for $\theta_1 > 0$) (@fig-nlr-redterm).  
...
plot(t,r1,type="l",ylab="1-exp(-theta t)",las=1,bty="l")
lines(t,r2)
lines(t,r3)
```
::::
...
A central issue in fitting nonlinear regression models is the choice of starting values. The algorithm kicks off with initial estimates $\btheta^{(0)}$ and updates them (iterates) until the minimum of SSE is found. Starting values that are close to the solution greatly facilitate the estimation. Starting values far from the solution require more iterations and increase the odds to find a local minimum.

A number of techniques can be used to find starting values for the parameters and these are often used in combination.
We illustrate with an example from modeling plant yield with the Mitscherlich equation and simulated data.
...
The following code simulates 21 observations from a Mitscherlich model with
$\lambda = 40$, $\xi = 40$, and $\kappa = 0.01$.
...
::::
::::{.example-container}
The Mitscherlich equation has a lower and upper bound of yield. The lower bound $\xi$ occurs at $x=0$. Based on the data points in @fig-mitsch-data, starting values of $\xi^{(0)} = 40$ and $\lambda^{(0)} = 80$ are reasonable.
::::
:::
...
inspection of the data. Finding a good starting value for $\kappa$ is trickier. We can lean on
the math to find a value using linear regression.
...
-   $\xi^{(0)}$ = 80 - exp(`{r} round(coef(lm.fit)[1],4)`) = `{r} round(80 - exp(coef(lm.fit)[1]),4)`
...
1. Set up a grid of starting values and call `nls2` with `algorithm="grid-search"`. This will return as the coefficients of a nonlinear regression object the best set of starting values.
2. Call `nls` with the best set returned from `nls2` as the starting values or call `nls2` again, passing the best set from the grid search as the starting values.
...
The algorithm coverges very quickly, after only three iterations. 
...
frame that defines the bounding box for the starting values and control the density of
the grid with the `maxiter` parameter of `nls.control()`.
...
Like @Davidian1995, we eliminate the data points at time $t=0$ from the analysis of a one-compartmental model with first-order absorption and elimination. This model expresses $C(t)$, the serum concentration at time $t$ following administration of dose $D$, as 
$$
C(t) = \frac{D k_a}{V(k_a - Cl/V)}\left\{\exp\left(-\frac{Cl}{V}t\right) - \exp(-k_a t) \right\}
$$ {#eq-compartmental1}

The parameters to be estimated in @eq-compartmental1 are $\{k_a,V,Cl\}$, the rate of absorption $k_a$, the volume of distribution $V$, and the clearance $Cl$.
...
Thankfully, the estimates for all parameters are positive; a negative absorption, negative clearance, or negative volume would be difficult to explain. The positivity constraint can be built into the model through reparameterization. 
Instead of estimating $k_a$, $Cl$, and $V$, the model can be written in terms of $\beta_1 = \log Cl$, $\beta_2 = \log k_a$, and $\beta_3 = \log V$.
...
The **expected value parameterization** introduced by Ratkowsky [-@Ratkowsky1983; -@Ratkowsky1990] reduces the parameter-effects curvature which he argues is the greater of the two curvature components.

Suppose the model has a single input variable, $x$. By visual inspection based on the scatterplot of $Y$ versus $x$ you can obtain a guesstimate of $\Exp[Y|x^*]$ for a particular value $x^*$ of the input. Call this estimate $\mu^*$ and set it equal to $f(x^*,\btheta)$. You can now replace one of the parameters with $\mu^*$ for which you already have a starting value. An example from @Schabenberger2001 [p.209] will make this approach more tangible.
...
\Exp[Y|x] = \frac{Vx}{x+K}
$$ {#eq-michaelis-menten}
$Y$ is the velocity of the chemical reaction, $x$ is the substrate concentration. The parameters $V$ and $K$ measure the theoretical maximum velocity ($V$) and the substrate concentration at with velocity $V/2$ is attained ($K$).
...
$$
Instead of the parameters $\btheta = [V,K]$ you are now estimating $\btheta^* = [\mu^*,K]$. This model has the same intrinsic curvature as @eq-michaelis-menten but has lower parameter-effects curvature and the starting value for $\mu^*$ is easily found by graphical inspection.
...
The expected-value reparameterization does not lead to a more readable form of the model, we choose a value $x^*$ and replace one of the parameters with $\mu^*$. @Schabenberger2001 describe the reverse process, choosing a value $\mu^*$ and replacing a model parameter with the input value $x^*$ that corresponds to it. This is termed reparameterization through **defining relationships** and is useful when the goal is to estimate $x^*$, an inverse prediction problem.
...
\Exp[Y|x^*] = f(x^*,\btheta)
$$
but now we are solving for $x^*$ in the quest to replace a parameter and not for $\Exp[Y|x^*]$.
...
$$
The asymptotes of the growth model are $\alpha$ and $\delta$ and growth transitions in sigmoidal shape between those. 
Define $\lambda_K$ as the the value at which $K\%$ of the difference between lower and upper asymptote is reached
$$
...
For $K=50$, $\lambda$ is the point on the $x$-axis where the difference between upper and lower asymptote is reduced by half, $\widehat{\lambda}=$ `{r} round(coef(fit)[3],4)`. We can validate that easily:
...
@fig-sim-log-logistic_50 displays observed and fitted values along with the $\widehat{\lambda}_{50}$ value.

```{r, echo=FALSE, fig.align="center", out.width="90%"}
#| fig.cap: Observed and fitted values with $\lambda_{50}$
#| label: fig-sim-log-logistic_50
plot(df$x,df$y,las=1,bty="l",xlab="x",ylab="y")
lines(df$x,predict(fit),lwd=2)
...
summary(fit2)
```
Notice that the parameter estimates of $\alpha$, $\delta$, and $\beta$ do not change, and the overall fit is the same, as can be seen from the residual standard error. The estimate of the point where the difference between the asymptotes is reduced by 75\% is $\widehat{\lambda}_{75}=$ `{r} round(coef(fit2)[3],4)`.
