...
All we need to turn the predictions from a logistic regression model into a classification model is a rule that assigns predictions to events or non-events based on the predicted probabilities. The **cutoff** $c = 0.5$ is common and is known as the **Bayes classifier**. If a predicted probability is greater than 0.5 it is classified as an event, otherwise it is classified as a non-event. More generally, the Bayes classifier is the rule that assigns the category for which the predicted probability is highest. With two categories, this is equivalent to classifying an event if the predicted probability exceeds 0.5. The Bayes classifier is optimal in that it has the smallest probability of a misclassification.
...
::::
::::{.definition-container}
The confusion matrix is the cross-classification between the observed and the predicted categories in a classification problem. If there are two categories, the confusion matrix is a 2 x 2 matrix.
...
The following table shows the layout of a typical confusion matrix for a 2-category classification problem. A false positive prediction, for example, is to predict a positive (“Yes”) result when the true state (the observed state) was negative. A false negative result arises when the decision rule assigns a “No” to an observation with a positive ("Yes") state.
...
A number of statistics are calculated based on the cells in the confusion matrix.

|                               |                                         |                                                                        |
|-------------------------------|-----------------------------------------|------------------------------------------------------------------------|
| **Statistic**                 | **Expression**                          | **Notes**                                                              |
...
: Statistics calculated from a 2 x 2 confusion matrix.
...
Probably the most frequently used confusion statistic is accuracy, the overall proportion of predicting correctly. 
...
The following table arranges the results in the format of the definition.
...
: Confusion matrix for test data in credit default example.
