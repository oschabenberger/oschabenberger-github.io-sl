...
$\phi^{R}(\bx)$ is the predicted value $\widehat{y} = \widehat{\bbeta}\bx$.

The point is that $\phi(\bx)$ is a random variable because it depends on the 
particular sample (the training data) for $Y$ and its statistical properties are
...
estimation method, it is very stable. Bagging such a a regression estimator does not
