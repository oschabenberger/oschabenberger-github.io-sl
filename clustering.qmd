...
Probably the most frequent choice, but it is sensitive to large distances due to squaring.

+ **Absolute distance**, also called "Manhattan" or city-block distance
		$$d_j(x_{ij},x_{i^\prime j}) = |x_{ij} - x_{i^\prime j}|$$
Absolute distance is more robust to large differences compared to dissimilarity based on Euclidean distance.
...
The dendrogram for this analysis is plotted in @fig-glaucoma-dendro1 along with the bounding boxes for 4 and 8 clusters. The cut for the larger number of clusters occurs lower at the tree.
...
Changing the linkage method to single demonstrates the *chaining* effect on the dendrogram (@fig-glaucoma-dendro2). Identifying a reasonable number of clusters is more difficult.
